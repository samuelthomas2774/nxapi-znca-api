...
The `X-znca-Client-Version` header is used to check if the client may be incompatible with newer coral versions and should never be set dynamically. From 3.1.0, this is a random per-client value issued by nxapi-auth.
